...
/**
 * Safe colour grade: saturation + contrast around mid-grey with a clamp to non-negative values
 * (the stock HueSaturationEffect produced negative channels → NaN/black on saturated colours).
 */
...
void mainImage(const in vec4 inputColor, const in vec2 uv, out vec4 outputColor) {
  ...
  c = max((c - 0.5) * (1.0 + contrast) + 0.5, 0.0);
  outputColor = vec4(c, inputColor.a);
}`, { uniforms: new Map<string, THREE.Uniform>([['saturation', new THREE.Uniform(0.1)], ['contrast', new THREE.Uniform(0.06)]]) });
  }
...
export class Post {
  ...
  constructor(private renderer: THREE.WebGLRenderer, private scene: THREE.Scene, private camera: THREE.PerspectiveCamera, q: QualityProfile) {
    ...
    if (q.ao) {
      ...
      Object.assign(this.ao.configuration, {
        aoRadius: 2.2, distanceFalloff: 0.8, intensity: 2.6, aoSamples: q.aoHalfRes ? 12 : 16, denoiseSamples: 6, denoiseRadius: 10,
  ...
  setNight(n: number): void {
    ...
    this.grade.saturation = 0.1 - n * 0.22;
  }

  render(dt: number, lowHealth: number): void {
    this.hitPulse = Math.max(0, this.hitPulse - dt * 2.5);
    const off = 0.0012 + this.hitPulse * 0.006 + lowHealth * 0.002;
    this.chroma.offset.set(off, off * 0.6);
    this.vignette.darkness = 0.55 + this.hitPulse * 0.3 + lowHealth * 0.3;
