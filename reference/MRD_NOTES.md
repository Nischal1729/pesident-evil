...
## 5. Interiors that could be enterable at ground level

None of the MRD interiors are at the flat-world ground level: the entrance hall/atrium sits at +1.8 m (top of the
grand steps) and the OAT-side entrances at +3.2 m. When multi-level movement lands, the forecourt podium (+1.8 m, tag
`mrd:forecourt`) and the steps ramp (`mrd:steps`) are already registered; the atrium (≈20 × 16 m, 4 storeys,
octagonal skylight, navy columns, red chairs) is the obvious first interior, entered from the forecourt doors.
